...
public sealed class DomainPropertyTests
{
    ...
    public Property Normalized_boxes_are_order_independent(int x1, int y1, int x2, int y2)
    {
        ...
        return (forward == reverse && forward.Width >= 0 && forward.Height >= 0).ToProperty();
    }

    [Property(MaxTest = 1000, QuietOnSuccess = true)]
    public Property Dpi_scaling_round_trip_is_within_half_a_pixel(short dipValue, PositiveInt dpiValue)
    {
        var dip = dipValue / 4d;
        ...
        return (Math.Abs(roundTrip - dip) <= ScreenCaptureGeometry.DefaultDpi / dpi / 2d + 1e-9).ToProperty();
    }

    [Property(MaxTest = 1000, QuietOnSuccess = true)]
    public Property Export_file_names_are_portable_bounded_and_idempotent(NonNull<string> input, PositiveInt limitValue)
    {
        ...
        return (sanitized.Length is > 0 && sanitized.Length <= limit &&
                ...
                sanitized == ExportFileName.Sanitize(sanitized, maximumLength: limit)).ToProperty();
    }

    [Property(MaxTest = 500, QuietOnSuccess = true)]
    public Property Portable_configuration_round_trip_preserves_non_transient_values(NonNull<string> language, bool images, PositiveInt side)
    {
        ...
        return (restored.Global == source.Global && restored.SchemaVersion == source.SchemaVersion && restored.Secrets.Count == 0).ToProperty();
    }

    [Property(MaxTest = 500, QuietOnSuccess = true)]
    public Property Sections_are_composed_in_order_then_by_stable_identifier(NonEmptyArray<int> orders)
    {
